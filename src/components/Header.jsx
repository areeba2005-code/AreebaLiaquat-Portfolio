      ...
      <div className='max-w-7xl mx-auto flex justify-between items-center'>
        ...
        <nav className='hidden md:flex items-center gap-10'>
          <ul className='flex gap-8'>
            {menuItems.map((item) => ( // Fixed: changed 'items' to 'item'
              <li key={item.name}>
                <button
        ...
        {/* Mobile Menu Toggle */}
        <button
          className='md:hidden text-white' // Fixed: changed md-hidden to md:hidden
          onClick={() => setIsMenuOpen(!isMenuOpen)}
        >
          {isMenuOpen ? <X size={32} /> : <Menu size={32} />} {/* Fixed: lowercase 'size' */}
        </button>
      </div>
      {isMenuOpen && (
    ...
    <ul className='flex flex-col gap-8'>
      ...
      <li className='pt-6'>
        <button onClick={() => scrollToSection('#contact')}
          className='w-fu;; bg-primary text-white
